 ...
 * Architecture : 4 dictionnaires (4 lettres facile/difficile, 5 lettres facile/difficile)
 * Logique : le dico "difficile" est un supplément ajouté au dico "facile"
 * Les deux dicos (facile + difficile) servent toujours à valider les propositions
 */
...
// Supplément difficile 4 lettres (ajouté à facile pour le mode difficile)
...
// Supplément difficile 5 lettres
const dico5_suppl = ['ABCES','ACERB','ALESE','AMBLE','ARETE','ATOLL','AZYME','BAFFE','BARDE','BERME','BERNE','BESAC','BETEL','BIJOU','BIOME','BISON','BLINI','BOCCE','BOGEY','BONZE','BORAX','BOYAU','BUGLE','BULBE','BUTOR','CABOT','CAIEU','CALAO','CAMEE','CAMPO','CAPRE','CAREX','CASTE','CAURI','CHAPE','CHAUX','CHYLE','CIPPE','CIRRE','CLAIE','COLZA','COPAL','CORNU','COXAL','CRABE','CROUP','CTENE','CULEX','CYCAS','DARSE','DECOR','DECRI','DELIT','DELTA','DEUCE','DIETE','DIGIT','DILUE','DIVOT','DOGME','DOLCE','DOLME','DORIS','DOUVE','DROME','DUCAT','EDILE','ELITE','EMERI','EMULE','ENCAS','ENCAN','ENFEU','ENTAM','ENVOI','EPODE','EPURE','ERGOT','ESPAR','ETALE','EVASE','EXODE','EXPAT','FADET','FAGNE','FALOT','FASCE','FAUDE','FAVEL','FECAL','FELON','FENEC','FERAL','FIOLE','FLUET','FOEHN','FOLIO','FORET','FOYER','FRELE','FRENE','FRISE','FURET','GAFFE','GAINE','GAIZE','GALOP','GAMME','GAUFE','GAULE','GENET','GERCE','GESTE','GIBET','GIBUS','GLANE','GLEBE','GLOME','GODET','GOLEM','GOUGE','GOURE','GREVE','GRIOT','GUERE','GYBBE','HANAP','HARET','HELIN','HERPE','HILUM','HOBEU','HOCHA','HOGNA','HOTTE','HUARD','HUPPE','ICTUS','IMAGE','IMAGO','INDRE','INFUS','INVAR','ISARD','IVREE','JABOT','JACOT','JARRE','JASPE','JAVEL','JETON','JOUAL','JOUTE','JUIVE','JUTEX','KNOUT','KRACH','LABEL','LABRE','LACIS','LAITE','LAIUS','LAMPE','LARVE','LAVIS','LAXER','LECHE','LEMME','LEPRE','LESTE','LIARD','LILAS','LIMON','LODEN','LOPIN','LOSER','LOUET','LURON','LUTER','MACLE','MAGMA','MAHON','MALIN','MAOUS','MAREE','MAROT','MARTE','MATIR','MAUVE','MAZOT','MECHE','MEDOC','MELEE','MELON','MENOU','MEROU','MESOB','METIS','MIETE','MOREL','MOTET','MOULE','MOULT','MOYEN','MUSEE','MUGER'];
...
/**
 * Initialise une partie avec les paramètres choisis par l'utilisateur.
 * @param {number} nbLettres - 4 ou 5
 * @param {string} niveau    - 'facile' ou 'difficile'
 */
function demarrerPartie(nbLettres, niveau) {

    // --- Construction des dicos actifs ---
    let dicoFacile, dicoSuppl;
    if (nbLettres === 4) {
        dicoFacile = dico4_facile;
        dicoSuppl  = dico4_suppl;
    } else {
        dicoFacile = dico5_facile;
        dicoSuppl  = dico5_suppl;
    }

    // Le dico de validation accepte TOUJOURS les deux listes (évite les faux "inconnu")
    const dicoValidation = new Set([...dicoFacile, ...dicoSuppl]);

    // Le dico de tirage dépend du niveau
    const dicoPioche = (niveau === 'difficile')
        ? [...dicoFacile, ...dicoSuppl]
        : dicoFacile;

    // --- Tirage du mot ---
    const motSecret = dicoPioche[Math.floor(Math.random() * dicoPioche.length)];

    // --- Réinitialisation de l'interface ---
    document.getElementById('nav').style.visibility = 'hidden';
    document.getElementById('zone1').style.display   = 'flex';
    ...
    document.getElementById('input1').maxLength         = nbLettres;
    document.getElementById('input1').placeholder       = `Entrez un mot de ${nbLettres} lettres`;
    ...
    document.getElementById('titrePartie').textContent =
        `${nbLettres} lettres — ${niveauLabel} (${dicoPioche.length} mots)`;
    ...
    // Retirer l'ancien handler pour éviter les empilements
    const newForm = form.cloneNode(true);
    form.parentNode.replaceChild(newForm, form);
    const inputActif = document.getElementById('input1');
    ...
    newForm.onsubmit = (e) => {
        ...
        const proposition = inputActif.value.toUpperCase().trim();

        // Vérification de longueur
        if (proposition.length !== nbLettres) {
            err.textContent = `Le mot doit faire ${nbLettres} lettres`;
            err.style.visibility = 'visible';
            return;
        }

        // Vérification dans le dictionnaire — ne PAS incrémenter le compteur si inconnu
        if (!dicoValidation.has(proposition)) {
            err.textContent = 'Ce mot n\'appartient pas au dictionnaire';
            err.style.visibility = 'visible';
            inputActif.value = '';
            return; // ← compteur NON incrémenté
        }

        // Mot valide : on incrémente
        cpt++;

        // Comptage des lettres bien placées
        let count = 0;
        ...
        if (count === 0)               texte.classList.add('zero');
        else if (count === 1)          texte.classList.add('une');
        else if (count === 2)          texte.classList.add('deux');
        else if (count === nbLettres - 1) texte.classList.add('trois');
        else if (count === nbLettres)  texte.classList.add('quatre');

        ligne.textContent  = proposition + ' : ';
        texte.textContent  = count;
        ligne.appendChild(texte);
        pave.appendChild(ligne);

        // Victoire
        if (count === nbLettres) {
            ligne.classList.remove('lignes');
            ligne.classList.add('degrade');
            gagne.querySelector('#nombreCoups').textContent = ` ${cpt} coup${cpt > 1 ? 's' : ''} ! Bravo !`;
...
function choixprop(form4) {
    // Lecture des deux groupes de radio
    const longueur = parseInt(document.querySelector('input[name="longueur"]:checked').value);
    const niveau   = document.querySelector('input[name="niveau"]:checked').value;
    demarrerPartie(longueur, niveau);
}
